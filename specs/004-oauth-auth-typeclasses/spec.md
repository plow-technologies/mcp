...
- **AuthUser**: User type for the reference in-memory implementation we ship. Defined in `MCP.Server.Auth.Demo` alongside the demo `AuthBackend` instance. NOT a "default" - it is one example among equals. Custom implementations define their own user types via the `AuthBackendUser m` and `OAuthUser m` associated types; the type equality constraint ensures they agree. Must NOT be imported by polymorphic handler code.
- **UserId (reference)**: User identifier type for the reference in-memory implementation. Defined in `MCP.Server.Auth.Demo` (or `MCP.Server.OAuth.InMemory` if shared). Custom implementations define their own via `AuthBackendUserId m` and `OAuthUserId m` associated types. Must NOT be imported by polymorphic handler code.
