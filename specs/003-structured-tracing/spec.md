...
- Q: Should tracers be stored in config structs or passed as arguments? → A: Always passed as function arguments, NEVER stored in config structs. This avoids confusion (two sources of truth) and allows callers to explicitly contramap their tracers for callees. Application creates the tracer; library code threads it (contramapped or not) or uses it to trace events. Clean composition requires explicit tracer threading.
